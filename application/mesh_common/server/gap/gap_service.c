...
/* Device name - Extra two octets for storing AD Type and a Null ('\0') */ 
 uint8 g_device_name[DEVICE_NAME_LENGTH + 2];
...
static void serviceDataInit(void)
{
    ...
    g_service_data.length = StrLen("@MGXXXX");

    g_device_name[0] = AD_TYPE_LOCAL_NAME_COMPLETE;
    MemCopy(p_name, appearance.shortName, g_service_data.length);
 ...
 *
 *  DESCRIPTION
 *      This function initialises the GAP service in case where bonding is 
 *      not required
 *
 *  RETURNS
 *      Nothing.
